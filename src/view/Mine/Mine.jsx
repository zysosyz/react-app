...
import { Arrow, PendingPayment, Logistics, SendGift, Comment, Edit } from '@react-vant/icons'
import './mine.css'
...
function Mine() {
    ...
    useEffect(() => {
        ...
        // http()
        // if (files) {
        //     http('/upload/image?model=maintain&pid=0', 'post', files, 'multipart/form-data').then(res=>{
        //         console.log(res);
                ...
                <div className='head'>
                    <div className='head-top'>
                        {msg &&
                            <Cell
                                ...
                                label={<div>
                                    <span>{msg.phone}</span>
                                    <span style={{ marginLeft: '5px' }}>{<Edit fontSize={16} style={{ verticalAlign: 'top' }} />}</span>
                                </div>}
                                icon={
                                    <>
                                        <Image lazyload={true} loadingIcon={<Loading type='spinner' />} width={60} height={60} src={msg.avatar} round onClick={() => {
                                            ref.current.click()
                                        }} />
                    ...
                    <div className='order-wrapper'>
                        <div className='center'>
                            <span style={{ fontSize: '15px', color: '#282828', fontWeight: 'bolder' }}>订单中心</span>
                            <span style={{ fontSize: '13px', color: '#666' }} onClick={() => {
                                navigate('/user/orderlist')
                            }} >查看全部<Arrow /></span>
                        </div>
                        <div className='order-bd'>
                            {Msg.map((item, index) =>
                                <div className='bd-item' key={index} onClick={() => {
                                    navigate('/user/orderlist', {
                                        state: {
                                            index
                                        }
                            ...
                            <Swiper autoplay={3000} >
                                {
                                    list.map((item, index) =>
                                        <Swiper.Item key={item.id}>
                                            <div className='pic'>
                                                <Image lazyload={true} loadingIcon={<Loading type='spinner' />} src={item.pic}></Image>
                    ...
                    <div className='server'>
                        ...
                        <div className='list-box'>
                            {serverList?.map((item, index) =>
                                <div className='server-item' key={item.id} onClick={() => {
                                    if (item.name === '地址信息') {
                                        // console.log(111);
                                        navigate('/address')
                                    }
                                    ...
                                    <div className='img' >
                                        <Lazyimg className='lazy' src={item.pic} placeholder={<Loading type='spinner' />}></Lazyimg>
                                    </div>
                                    <span>{item.name}</span>
                                </div>
                            )}
